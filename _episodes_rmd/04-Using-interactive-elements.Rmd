...
```{r car_plot, fig.width=6, fig.height = 5}
cars <- mtcars %>% 
...
```{r pretend_run_plotly_basic, eval=FALSE, echo = T}
ggplotly(car_plot)
```

```{r save_plotly_basic, include=F}
# ggplotly won't work on github, so save and load it in an iframe to pretend it worked
htmlwidgets::saveWidget(plotly::ggplotly(car_plot), "../data/plotly_basic_example.html")
```
...
```{r pretend_run_plotly_with_colours, echo = TRUE, eval = FALSE}
...
```{r save_plotly_with_colours, include=F}
# ggplotly won't work on github, so save and load it in an iframe to pretend it worked
colourful_plot <- ggplot(cars, aes(x = wt, y = mpg, colour = cyl)) + 
  geom_point()

htmlwidgets::saveWidget(ggplotly(colourful_plot), "../data/plotly_colours_example.html")
```
...
```{r pretend_run_plotly_tooltip, echo = TRUE, eval = FALSE}
...
```{r save_plotly_tooltip, include=F}
# ggplotly won't work on github, so save and load it in an iframe to pretend it worked
tooltip_plot <- ggplot(cars, aes(x = wt, y = mpg, colour = cyl, text = car_name)) + 
  geom_point()

htmlwidgets::saveWidget(ggplotly(tooltip_plot, tooltip = "text"), "../data/plotly_tooltip_example.html")
```
